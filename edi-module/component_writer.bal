...
function writeComponentGroup(json componentGroup, EDISegSchema segSchema, EDIFieldSchema fieldSchema, EDIContext context) returns string|Error {
    if componentGroup is () {
        if fieldSchema.required {
            return error Error(string `Mandatory composite field "${fieldSchema.tag}" of segment "${segSchema.tag}" is not provided.`);
        } else {
            return "";
        }
    }
    ...
    if componentGroup is map<json> {
        ...
        if ckeys.length() < fieldSchema.components.length() && !fieldSchema.truncatable {
            return error Error(string `Field ${fieldSchema.tag} in segment ${segSchema.code} must have ${fieldSchema.components.length()} components. 
        ...
        while cindex < fieldSchema.components.length() {
            ...
            if cindex >= ckeys.length() {
                if cmap.required {
                    return error Error(string `Mandatory component ${cmap.tag} not found in ${componentGroup.toString()} in segment ${segSchema.code}`);
                }
                cindex += 1;
                continue;
            }
            ...
            if ckey != cmap.tag {
                return error Error(string `Component ${cmap.tag} - cindex: ${cindex} [segment: ${segSchema.tag}, field: ${fieldSchema.tag}] in the schema does not match with ${ckey} found in the input EDI.`);
            }
            var componentValue = componentGroup.get(ckey);
            if componentValue is string && componentValue.trim().length() == 0 {
                if cmap.required {
                    return error Error(string `Mandatory component ${cmap.tag} in [Segment: ${segSchema.code}, Field: ${fieldSchema.tag}] not provided`);
                } else {
            ...
            if cmap.subcomponents.length() == 0 {
                if componentValue is SimpleType {
                    ...
                    return error Error(string `Component ${cmap.tag} in [Segment: ${segSchema.code}, Field: ${fieldSchema.tag}] must contain a primitive value. Found: ${componentValue.toString()}`);
                }
                ...
                if scGroupText is string {
                    ...
                    return error Error(string `Component ${cmap.tag} in [Segment: ${segSchema.code}, Field: ${fieldSchema.tag}] must contain a composite value. Found: ${componentValue.toString()}`);
                }
            } else {
                return error Error(string `Unsupported component value. Found ${componentValue.toString()}`);
            }
        }
        return cGroupText;
    } else if componentGroup is string && componentGroup.trim() == "" {
        if fieldSchema.required {
            return error Error(string `Mandatory compite field ${fieldSchema.tag} of segment ${segSchema.tag} is not available in the input.`);
        } else {
            return "";
        }
    } else {
        return error Error(string `Input segment is not compatible with the schema ${printSegMap(segSchema)}.
...
function writeSubcomponentGroup(json subcomponentGroup, EDISegSchema segSchema, EDIComponentSchema compSchema, EDIContext context) returns string|Error {
    ...
    if subcomponentGroup is map<json> {
        ...
        if sckeys.length() < compSchema.subcomponents.length() && !compSchema.truncatable {
            return error Error(string `Component ${compSchema.tag} in segment ${segSchema.code} must have ${compSchema.subcomponents.length()} subcomponents. 
        ...
        while scindex < compSchema.subcomponents.length() {
            ...
            if scindex >= sckeys.length() {
                if scmap.required {
                    return error Error(string `Mandatory subcomponent ${scmap.tag} not found in ${subcomponentGroup.toString()} in segment ${segSchema.code}`);
                }
                scindex += 1;
                continue;
            }
            ...
            if sckey != scmap.tag {
                return error Error(string `Subcomponent ${scmap.tag} - scindex: ${scindex} [segment: ${segSchema.tag}, component: ${compSchema.tag}] in the schema does not match with ${sckey} found in the input EDI.`);
            }
        ...
        if compSchema.required {
            return error Error(string `Mandatory sub-compite field ${compSchema.tag} of segment ${segSchema.tag} is not available in the input.`);
        } else {
            return "";
        }
    } else {
